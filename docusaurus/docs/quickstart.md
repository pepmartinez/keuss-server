...
## Install & run
### As Docker Container
Easiest way is to use the docker image availabel at [docker hub](https://hub.docker.com/repository/docker/pepmartinez/keuss-server):
...
```sh
docker run --rm -d -p 3444:3444 -p 61613:61613 -v /opt/ks/etc:/usr/src/app/etc --name keuss-server pepmartinez/keuss-server:1.5.1
```
