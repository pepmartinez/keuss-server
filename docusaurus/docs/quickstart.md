...
```sh
docker run --rm -d --net host --name keuss-server pepmartinez/keuss-server:1.6.6
```
...
```sh
docker run --rm -d -p 3444:3444 -p 61613:61613 -v /opt/ks/etc:/usr/src/app/etc --name keuss-server pepmartinez/keuss-server:1.6.6
```
